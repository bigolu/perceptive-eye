...
all_color_images = [np.rollaxis(x, -1) for x in all_color_images]
all_color_images = np.array(all_color_images)
all_depth_images = []
for i in range(num_items):
...
reshaped = mx.sym.reshape(name="reshaped", data=relu6, shape=(64, 30, 40))

dec1 = mx.sym.Deconvolution(
    data=reshaped,
    kernel=(4,4),
    stride=(2,2),
    pad=(1,1),
...
depth = mx.sym.LinearRegressionOutput(
    data=conv8,
    name="depth"
    )
...
nb_epoch = 50
model.fit(train_iter, num_epoch=nb_epoch, \
